...
from artcode.core.tool import ApprovalChoice, McpLoading, PermissionMode, RunMode, ToolCall
...
@pytest.mark.parametrize("loading", (McpLoading.EAGER, McpLoading.LAZY))
async def test_real_streamable_http_initializes_discovers_calls_and_closes(
    tmp_path: Path, loading: McpLoading,
) -> None:
    ...
    try:
        ...
        if loading is McpLoading.LAZY:
            assert not any(item.name.startswith("mcp__") for item in run.descriptors)
            search = await tools.execute_batch(run, (
                ToolCall("search", "mcp_search_tools", '{"query":"echo","limit":1}'),
            ))
            assert search[0].ok and json.loads(search[0].content)["tools"][0]["active"]
            run = tools.refresh_run(run)
        else:
            assert "mcp_search_tools" not in {item.name for item in run.descriptors}
