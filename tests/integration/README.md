...
- `tests/live/`：真实模型服务、macOS Seatbelt、进程树、stdio/HTTP MCP、CLI 和多进程 Session。
...
真实模型测试读取 `~/.artcode/config.yml` 或项目根目录 `artcode.yaml`，并直接使用其中的模型与服务地址。真实 CLI 测试使用隔离 HOME、临时 Workspace 和测试配置，在不发送模型请求的情况下验证 console script、`python -m artcode`、新会话和精确恢复走同一 Bootstrap。
