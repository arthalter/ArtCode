...
from .chta.runner import ChTARunner, regenerate_chta_report
from .manifest import BenchmarkValidationError, load_benchmark, load_chta_manifest
...
def build_parser() -> argparse.ArgumentParser:
    ...
    chta = subcommands.add_parser("chTA", help="运行 chTA 系统效率与信息保真评测")
    chta_commands = chta.add_subparsers(dest="chta_command", required=True)
    chta_validate = chta_commands.add_parser("validate", help="严格校验 chTA 清单")
    chta_validate.add_argument("manifest", type=Path)

    chta_run = chta_commands.add_parser("run", help="运行 chTA A/B 实验")
    chta_run.add_argument("manifest", type=Path)
    chta_run.add_argument(
        "--experiment",
        choices=("mcp", "permission", "swebench", "all"),
        default="all",
    )
    chta_run.add_argument("--config", type=Path, help="真实模型配置；MCP/all 必需")
    chta_run.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/chTA-real"),
        help="chTA 运行根目录",
    )

    chta_report = chta_commands.add_parser("report", help="重新生成 chTA 报告")
    chta_report.add_argument("run_dir", type=Path)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "compare":
            ...
            return 0
        if args.command == "chTA":
            if args.chta_command == "validate":
                manifest = load_chta_manifest(args.manifest)
                print(
                    f"chTA 清单有效：{manifest.name} | "
                    f"experiments={len(manifest.experiments)} | "
                    f"sha256={manifest.fingerprint}"
                )
                return 0
            if args.chta_command == "report":
                json_path, markdown_path, workbook_path = regenerate_chta_report(
                    args.run_dir
                )
                print(f"chTA 报告：{json_path}")
                print(f"Markdown：{markdown_path}")
                print(f"工作簿：{workbook_path}")
                return 0
            return asyncio.run(_run_chta(args))
...
async def _run_chta(args: argparse.Namespace) -> int:
    manifest = load_chta_manifest(args.manifest)
    repository = _repository_root(Path.cwd())
    if repository is None:
        raise ValueError("chTA 必须在 ArtCode Git 仓库内运行")
    runner = ChTARunner(
        manifest,
        output_root=args.output_dir,
    ...
    print(f"chTA 运行完成：status={result.status} | run={result.run_dir}")
    print(f"JSON：{result.report_json}")
    print(f"Markdown：{result.report_markdown}")
    if result.report_workbook is not None:
        print(f"工作簿：{result.report_workbook}")
    else:
        print(f"工作簿生成失败证据：{result.run_dir / 'report-workbook.error.txt'}")
