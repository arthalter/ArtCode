...
def convert_call_result(tool_name: str, result: Any) -> ToolResult:
    blocks: list[str] = []
    ...
    if getattr(result, "isError", False):
        return error_result(tool_name, "mcp_tool_error", "MCP Server 返回工具错误。", content)
    return success_result(tool_name, "MCP 工具执行成功。", content)
